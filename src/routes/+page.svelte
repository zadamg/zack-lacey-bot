<script lang="ts">
  ...
  async function handleHigherOracleClick() {
        ...
        try {
            ...
            // hide the form-container by slowly lowering the opacity
  ...
  async function handleDeconstructClick() {
    handleHigherOracleClick();

      const additionalData = {
              user: 'Profiler',
              queries: ['advertising production news (adweek, adage, digiday, etc.)', 'agencies and production companies using AI; new pricing models'],
...
<div class="biggie w-full h-auto overflow-hidden !bg-{backgroundColor}" style="background: {backgroundColor} !important">
  ...
  on:pointerleave={handleMouseOut}>
          ...
          <p class="m-4 text-{foregroundColor}" style="font-size: clamp(1rem,1.2vmax,2rem)">
            Ask anything about Zack & Lacey's relationship and get an honest answer from a neutral, therapeutic perspective...
          </p>
          {/if}
    ...
    {/if}
		<div class="container h-full mx-auto flex justify-center items-center mt-[-12vmax] xl:mt-[-5vmax]">
			...
			<form class="c-form relative" action="" autocomplete='off' id="form-container" on:pointerenter={handleMouseOver}  on:pointerleave={handleMouseOut} style="height: clamp(1em, 15%, 5em); width: clamp(16em, 75%, 50em); padding: 5px; ">
        
				<input class="c-form__input"  title="queryInput" id="queryInput" bind:value={queryInput} on:focus={handleMouseOver} spellcheck="false" on:pointerenter={handleMouseOver} on:pointerdown={handleMouseOver}  style="font-size: clamp(1.35rem, 1.8vmax, 2.5rem); { wavy ? 'filter:url(#wavy); margin-right: 5px; margin-bottom: 15px; color: rgb(207, 12, 12, 0.0); ' : '' } "  placeholder="{ wavy ? '' : '...don\'t be shy...'}" type="website" required pattern="^((https?|ftp|smtp):\/\/)?(www.)?[a-z0-9]+\.[a-z]+(\/[a-zA-Z0-9#]+\/?)*$">
...
<svg class:wavy={makeWavy} style="position:absolute; padding:10px">
	<defs>
		<filter id="wavy" x="0" y="0" width="110%" height="110%">
			<feTurbulence id="sea-filter" numOctaves="4" seed="2" baseFrequency="0.2 0.5" />
